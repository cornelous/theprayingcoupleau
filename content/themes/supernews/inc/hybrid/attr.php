 ...
 * @return array
 */
function supernews_entry_markup( $classes ) {
...
add_filter( 'post_class', 'supernews_entry_markup' );
 ...
 * @return string
 */
function supernews_comment_reply_link_filter( $link ) {
	return preg_replace( '/(<a\s)/i', '$1itemprop="replyToUrl"', $link );
}
add_filter( 'comment_reply_link', 'supernews_comment_reply_link_filter', 5 );
 ...
 * @return string
 */
function supernews_comments_popup_link_attributes( $attr ) {
	return ' itemprop="discussionURL"';
}
add_filter( 'comments_popup_link_attributes', 'supernews_comments_popup_link_attributes', 5 );
 ...
 * @return string
 */
function supernews_get_avatar( $avatar ) {
	return preg_replace( '/(<img.*?)(\/>)/i', '$1itemprop="image" $2', $avatar );
}
add_filter( 'get_avatar', 'supernews_get_avatar', 5 );