 ...
 * @since      1.0.0
 */
class SuperNews_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-supernews-recent posts-thumbnail-widget',
			'description' => __( 'Display recent posts with thumbnails.', 'supernews' )
		);

		// Create the widget.
		$this->WP_Widget(
			'supernews-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts Thumbnails', 'supernews' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'supernews_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'supernews_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'supernews_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'supernews' ),
			'limit'     => 5,
			'show_date' => false,
			'layout'    => 'standard'
		);
